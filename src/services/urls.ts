...
export const createShortURL = async (
  ...
  if (body.id) {
    ...
    if (current_record) {
      throw new httpError.Conflict("ID itu sudah ada di Database");
    }
  }
...
export const resolveURL = async (id: string, ip: string) => {
  ...
  if (!url) {
    throw new httpError.NotFound("URL tidak ditemukan");
  }
...
export const updateURL = async (
  ...
  if (url.user_id !== user_id) {
    throw new httpError.Unauthorized("Kamu tidak diizinkan mengubah URL ini");
  }
